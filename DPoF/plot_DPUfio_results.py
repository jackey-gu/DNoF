...
# ================== 配置区 ==================
# 请修改为你的实际路径
csv_file = r"C:\Users\jackey_gu\Desktop\脚本\DPIO\DPoF\DPU_nvmeof_40DPUsupplement_summary.csv"
output_dir = r"C:\Users\jackey_gu\Desktop\脚本\DPIO\DPoF\DPU_nvmeof_40CPU_fio_variable_benchmark_supplement\fio_plots_supplement"
...
def save_plot(fig, output_path):
    ...
    fig.savefig(output_path, format='pdf', bbox_inches='tight')  # 🔥 输出为 PDF
    plt.close(fig)
...
def plot_metric(df, test_type_key, x_field, metric_name, y_cols, ylabel):
    ...
    通用绘图函数
    :param df: 数据框
    :param test_type_key: 如 'numjobs', 'numjobs-bw'
    ...
    output_path = os.path.join(metric_dir, f"{metric_name.replace(' ', '_')}_vs_{test_type_key}.pdf")  # 🔥 .pdf
...
if __name__ == "__main__":
    ...
    # 只处理当前存在的测试类型
    test_types = {
        "numjobs": "numjobs",       # 随机读写
        "numjobs-bw": "numjobs"     # 顺序读写
    }
