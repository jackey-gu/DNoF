...
# 读取 CPU 和 DPU 的 CSV 文件
csv_cpu = r"c:/Users/jackey_gu/Desktop/脚本/DPIO/DPIO testjob/addbufferedCPU_direct_benchmark/buffered_direct_results_summary.csv"
csv_dpu = r"c:/Users/jackey_gu/Desktop/脚本/DPIO/DPIO testjob/addbufferedDPU_direct_benchmark/buffered_direct_results_summary.csv"
...
# 输出目录（统一输出到对比目录）
output_dir = r"c:/Users/jackey_gu/Desktop/脚本/DPIO/DPIO testjob/addbufferedCPU_direct_benchmark/CPUDPUfio_plots_enhanced"
...
# 预定义排序顺序
test_order = {
    "mixed": list(map(str, range(0, 51))),
    "buffered1_directN": list(map(str, range(0, 17))),
    "direct1_bufferedN": list(map(str, range(0, 17))),
}
    ...
    for metric_name, metric_info in metrics.items():
        ...
        for mode_group, modes, prefix in [
            ...
            for rw_mode in modes:
                ...
                for jobtype in ['Buffered', 'Direct']:
                    ...
                    for platform in ['CPU', 'DPU']:
                        ...
                        marker = MarkerStyle('o') if platform == 'CPU' else MarkerStyle('x')  # 根据平台选择标记点

                        label = f"{platform}{rw_mode} ({jobtype})"

                        if metric_name == "CPU Utilization":
                            ax.plot(df_plat[x_field], df_plat["cpu_usr"],
                                    label=f"{label} - User", color=color, linestyle=linestyle, alpha=0.9, marker=marker)
                            ax.plot(df_plat[x_field], df_plat["cpu_sys"],
                                    label=f"{label} - System", color=color, linestyle=linestyle, alpha=0.7, marker=marker)
                        else:
                            ...
                            ax.plot(df_plat[x_field], df_plat[data_col],
                                    label=label, color=color, linestyle=final_linestyle, linewidth=1.5, marker=marker)
