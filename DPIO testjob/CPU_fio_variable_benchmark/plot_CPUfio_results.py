...
# 读取数据
csv_file = r"c:/Users/jackey_gu/Desktop/脚本/DPIO/DPIO testjob/CPU_fio_variable_benchmark/CPUfio_results_summary_enhanced.csv"
df = pd.read_csv(csv_file)

# 输出目录
output_dir = r"c:/Users/jackey_gu/Desktop/脚本/DPIO/DPIO testjob/CPU_fio_variable_benchmark/fio_plots_enhanced"
...
def save_plot(fig, output_path):
    ...
    fig.savefig(output_path, bbox_inches='tight', format='pdf')  # 确保使用的是 'pdf'
    plt.close(fig)
...
def plot_metric(df, test_type, x_field, metric_name, y_cols, ylabel):
    ...
    output_path = os.path.join(metric_dir, f"{metric_name.replace(' ', '_')}_vs_{test_type}.pdf")  # 修改这里使文件扩展名为 .pdf
