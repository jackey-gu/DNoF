...
# ================== 配置区 ==================
DEVICE="/dev/nvme3n2"                            # 测试设备（确保未挂载）
OUTPUT_DIR="DPU_sync_fio_variable_benchmark"   # 输出目录
mkdir -p "$OUTPUT_DIR"
...
run_test() {
    ...
    for value in "${var_values[@]}"; do
        ...
        # 执行 FIO 命令（✅ 已移除最后的 --rw="$rw_mode"，避免重复）
        fio \
            --name=nvme4 \
            --filename="$DEVICE" \
            --group_reporting \
...
# --- ✅ 单独测试读写比例 (rwmixwrite) ---
echo "==> 开始测试读写混合比例: randrw + rwmixwrite"
for rwmix in "${RWMIXWRITE_VALUES[@]}"; do
    local output_file="$OUTPUT_DIR/test_RWMIXWRITE=${rwmix}_randrw.json"
    echo "==> 测试 RWMIXWRITE=$rwmix (randrw)"
