...
# 高队列深度场景
run_test "RWMIXWRITE-4K-128" "RWMIXWRITE_VALUES[@]" "--rwmixwrite=% --rw=randrw" "--bs=4k --numjobs=1 --iodepth=128 --cpus_allowed=1-1" "randrw"

# 低队列深度场景（iodepth=8）
run_test "RWMIXWRITE-4K-16" "RWMIXWRITE_VALUES[@]" "--rwmixwrite=% --rw=randrw" "--bs=4k --numjobs=1 --iodepth=16 --cpus_allowed=1-1" "randrw"

# --- 大块混合读写测试 ---
echo "==> 开始测试：64K 混合读写性能 (rwmixwrite)"
run_test "RWMIXWRITE-64K-8" "RWMIXWRITE_VALUES[@]" "--rwmixwrite=% --rw=randrw" "--bs=64k --numjobs=1 --iodepth=8 --cpus_allowed=1-1" "randrw"
