...
# 输入目录和输出文件（请根据实际路径修改）
input_dir = r"C:\Users\jackey_gu\Desktop\脚本\DPIO\DPIO testjob\addDPU_fio_variable_benchmark_supplement"
output_csv = r"C:\Users\jackey_gu\Desktop\脚本\DPIO\DPIO testjob\addDPU_fio_variable_benchmark_supplement\supplement_summary.csv"

# CSV 输出字段
fields = [
    "filename", "test_type", "rw_mode", "bs", "numjobs", "cpus_allowed", "iodepth",
...
# 参数排序顺序（用于排序输出）
test_order = {
    "numjobs": [str(i) for i in range(1, 33)],
    "nbcpu": [str(i) for i in range(1, 33)],
    "numjobs-bw": [str(i) for i in range(1, 33)],
    "nbcpu-bw": [str(i) for i in range(1, 33)],
}
