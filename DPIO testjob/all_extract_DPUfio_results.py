...
# 输入目录和输出文件
input_dir = r"C:\Users\jackey_gu\Desktop\脚本\DPIO\DPIO testjob\DPU_fio_variable_benchmark_supplement"  # 新增目录
output_csv = r"C:\Users\jackey_gu\Desktop\脚本\DPIO\DPIO testjob\DPU_fio_variable_benchmark_supplement\DPUfio_results_summary_enhanced_supplement.csv"

# 定义字段
fields = [
    "filename", "test_type", "rw_mode", "bs", "rwmixwrite", "numjobs", "cpus_allowed", "iodepth",
    "read_iops", "write_iops", "read_bw_MiB", "write_bw_MiB",  # ✅ 单位改为 MiB
...
test_type_map = {
    ...
    "numjobs": re.compile(r"^test_numjobs=.*$"),         # ✅ 匹配 test_numjobs=xx
    "nbcpu": re.compile(r"^test_nbcpu=.*$"),             # ✅ 匹配 test_nbcpu=xx
    "iodepth": re.compile(r"^test_iodepth=.*$"),
    "numjobs-bw": re.compile(r"^test_numjobs-bw=.*$"),   # ✅ 匹配 test_numjobs-bw=xx
    "nbcpu-bw": re.compile(r"^test_nbcpu-bw=.*$"),       # ✅ 匹配 test_nbcpu-bw=xx
}
...
test_order = {
    ...
    "numjobs": [str(i) for i in range(1, 33)],           # ✅ 拓展到 1~32
    "nbcpu": [str(i) for i in range(1, 33)],             # ✅ 拓展到 1~32
    "iodepth": ["1", "2", "4", "8", "16", "32", "64", "128"],
    "numjobs-bw": [str(i) for i in range(1, 33)],        # ✅ 拓展到 1~32
    "nbcpu-bw": [str(i) for i in range(1, 33)],          # ✅ 拓展到 1~32
}
...
def parse_job(job):
    ...
    write = job.get("write", {})
    return {
        "read_iops": read.get("iops", 0) / 1000,         # ✅ 单位转为 KIOPS
        "write_iops": write.get("iops", 0) / 1000,
        "read_bw_MiB": read.get("bw", 0) / 1024,         # ✅ 单位转为 MiB/s
        "write_bw_MiB": write.get("bw", 0) / 1024,
...
with open(output_csv, "w", newline="") as csvfile:
    ...
    for file in files:
        ...
        if test_type == "bs":
            ...
            bs_val = params.get("bs", "4k")
            rwmixwrite_val = params.get("rwmixwrite", "0")
            numjobs_val = params.get("numjobs", "")  # ✅ 变动参数
